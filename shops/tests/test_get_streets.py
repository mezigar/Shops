...
from shops.serializers import StreetSerializer
...
class GetStreetApiTestCase(APITestCase):
    def test_get_street(self):
        ...
        url1 = "http://127.0.0.1:8000/city/1/street/"
        response = self.client.get(url1)
        serializer_data = StreetSerializer([street_1, street_2], many = True).data
        self.assertEqual(HTTP_200_OK, response.status_code)
        self.assertEqual(serializer_data, response.data['streets'])

        url2 = "http://127.0.0.1:8000/city/2/street/"
        response2 = self.client.get(url2)
        self.assertEqual(HTTP_200_OK, response.status_code)
        self.assertEqual([], response2.data['streets'])

        url3 = "http://127.0.0.1:8000/city/3/street/"
